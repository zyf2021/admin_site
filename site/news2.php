		...
		<?php
			...
			switch ($act) {
				case 'home':
					...
					$num_rows = mysqli_num_rows($mysqli->query("SELECT * FROM news WHERE direct = 'Городская сфера'")) or die('error! Записей не найдено!');
					$num_pages = ceil($num_rows / $per_page);
			
					$query = "SELECT id,name,theme,direct,content FROM news WHERE direct = 'Городская сфера' LIMIT $start_page,$per_page";
