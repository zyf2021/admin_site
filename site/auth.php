...
$result_query_select = $mysqli->query("SELECT * FROM `users` WHERE `email` = '".$email."' AND `pass` = '".$password."'");
...
if(!$result_query_select){
    ...
    if($result_query_select->num_rows == 1){
        ...
        $result = $mysqli->query("SELECT `role`,`id` FROM `users` WHERE `email` = '".$email."'");
        $role = mysqli_fetch_array($result);
        #echo $role[0];
        //пытаемся проверить админ это или нет
        if ($role[0] == 1){
            $_SESSION['admin'] = true;
            $_SESSION['user'] = false;
            $_SESSION['id_user'] = $role[1];
            header("HTTP/1.1 301 Moved Permanently");
