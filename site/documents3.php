...
?>
	<div class="center_vertical">
			<div><h3>Правила землепользования и застройки города</h3></div>
		<?php
			...
			switch ($act) {
				case 'home':
					...
					$num_rows = mysqli_num_rows($mysqli->query("SELECT * FROM documents WHERE direct = 'Правила землепользования и застройки города'")) or die('error! Записей не найдено!');
					$num_pages = ceil($num_rows / $per_page);
			
					$query = "SELECT id,name,theme,direct,content FROM documents WHERE direct = 'Правила землепользования и застройки города' LIMIT $start_page,$per_page";
				...
				case 'read_documents':
					if (isset($_GET['id'])){
						$id = $_GET['id'];
						#echo $id;
						$result = $mysqli->query("SELECT `name`, `theme`, `direct`, `content`, `date_of_entry`, `date_of_approved` FROM `documents` WHERE `id` = $id") or die("ERROR");
